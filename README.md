...
Программа должна понимать, что мир вокруг меняется и она может расчитывать лишь на снимок состояния, но в большом количестве случаев этого достаточно.
 ...
 Чтобы избежать накладных расходов на потоки (threads) многие системы переходят на модель Event/Callback.
Но стиль основанный на обратных вызовах разрывает и запутывает поток управления. Логика оказывается
размазанной между callbackами - то что называется `callback hell`. 
Такие подходы как реактивное программирование, не меняют сути, и приводят к рекомендациям вроде -  не делайте
длительных операций в callbackах.

Для асинхронного программирования Rich Hickey написал библиотеку `core.async`, которая реализует
математическую модель Тони Хоара - Communicating Sequencial Processes. 
Это модель кооперативной многозадачности, реализованная в языке go-lang, occam и Java CSP.

Важными составляющими являются понятие channel и go-block.
channel - это буфферизированная очередь (на основе  java.util.concurrent) с фиксированным размером предназначенная для общения между потоками. Важным является то, что канналы (как первоклассные члены программы) разрывают жесткую связь между отправителем и получателем. В канналы может писать любое число потоков, также как и читать.
По определению операции с канналами блокируют нить исполнения. Однако если эти операции производятся внутри go-блока,
то исполняемый блок освобождает системный thread - паркуется, и уступает место другому go-блоку.

Таким образом вы можете запустить произвольное количество go-блоков, а выполнятся они будут на фиксированном пулле threadов. Технически go-block представляет макрос, который трансформирует свое тело в state machine, вставляя переходы
в местах пишущих или читающих из канналов. Это так называемый IoC, который 
